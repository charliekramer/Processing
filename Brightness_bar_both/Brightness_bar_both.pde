...
void draw() {
  ...
  if (locXBar > width) locXBar = 0;
  if (locXBar < 0) locXBar = width;
  ...
  for (int x = 0; x < img.width; x++) {
    for (int y = 0; y < img.height; y++ ) {
      ...
      float dBar = dist(x, y, locX, y);
      float d = dist(x, y, locX, locY);
      
      float adjustbrightness =   (1+sin(t))*125*(maxdist-dBar)/maxdist + (1+cos(t))*(maxdist-d)/maxdist;
      //float adjustbrightness =   -255*(maxdist-dBar)/maxdist + 255*(maxdist-d)/maxdist;
